...
if __name__ == '__main__':
    ...
    newmodel = mobilenetv2_w1(pretrained=True).cuda()
    newmodel.eval()
    ...
    calibration_loader = getSelfBuiltCalibrationData(args.dataset,
                            batch_size=args.batch_size,
                            path='F:\\imagenet\\',
                            for_inception=False)
    
    train_loader = getTrainingData(args.dataset,   #getCalibrationData(args.dataset,
                            batch_size=args.batch_size,
                            path='F:\\imagenet\\',
                            for_inception=False)
    ...
    #test performance of unprocessed model
    bar = Bar('original model test', max=len(test_loader))
    ...
    print('\n original model: Final acc: %.2f%% top5 acc: %.2f%%  (%d/%d)' % (acc*100, acc_top5*100, correct, total))
    
    
    
    # prune the model
    pruner = pruner.__dict__[args.pruner](model=model, newmodel=newmodel, testset=test_loader, trainset=calibration_loader,
                                          optimizer=optimizer, args=args, **kwargs)
    #print(pruner.model, pruner.newmodel)
    pruner.prune()
    #print("pruner trainset length:", len(pruner.trainset))
    #print("new model structure:", newmodel)

    '''
    #test performance of pruned model
    bar = Bar('pruned model test', max=len(test_loader))
    total, correct = 0, 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
    ...
    print('\n pruned model: Final acc: %.2f%%  (%d/%d)' % (acc*100, correct, total))
    '''
    ...
    print("finetuned accuracy", accfinetune)
    
    newmodel = pruner.newmodel
    
    
    
    # fuse the batch normalization layer and convolutional layer
    conv_layers = []
    bn_layers = []
    for module in newmodel.named_modules():
    ...
    #create parameters useful in quantization
    Rs = []
    QReLUs = []
    for i in range(len(conv_layers)-1):
        #print(i, conv_layers[i].out_channels)
        #print(i, conv_layers[i].weight.data.shape)
        r = torch.zeros((conv_layers[i].weight.data.shape[0], 2))
        qrelu = torch.ones(conv_layers[i].weight.data.shape)*6
        Rs.append(r)
        QReLUs.append(qrelu)
        
        
    bar = Bar('calibration to get Rs (using calibration set)', max=len(calibration_loader))
    total, correct = 0, 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(calibration_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
    ...
    print('\n Calibration,  pruned model: Final acc: %.2f%%  (%d/%d)' % (acc*100, correct, total))
    
    '''
    #test performance of pruned model and experience batch normalization fusion
    bar = Bar('pruned model + bn_fusion test', max=len(test_loader))
    total, correct = 0, 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
    ...
    print('\n After fusion of batch normalization layer and convolutional layer,  pruned model: Final acc: %.2f%%  (%d/%d)' % (acc*100, correct, total))
    '''
    
    #tunable imbalance activation functions
    ...
    #test performance of pruned and quantized model
    bar = Bar('Final Testing', max=len(test_loader))
    total, correct, top5 = 0, 0, 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
    ...
    file = r'testdata\\experimental_result.txt'
    with open(file, 'a') as f:
          f.write('\nl1normPruning prune ratio: %.2f tait:  %.2f Final acc: %.2f%%' % (args.pruneratio, args.t, acc*100))